...
fun uploadImageView(app: WitParkingApp, imageView: ImageView) {
    ...
    uploadTask.addOnFailureListener {
        object : OnFailureListener {
            override fun onFailure(error: Exception) {
                Log.v("Subscription", "uploadTask.exception" + error)
            }
        }
...
fun updateAllSubscriptions(app: WitParkingApp) {
    val userId = app.currentUser.uid
    val userEmail = app.currentUser.email
    val subscriptionRef = app.database.ref.child("subscriptions")
        .orderByChild("email")
    val usersubscriptionRef = app.database.ref.child("user-subscriptions")
        .child(userId).orderByChild("uid")

    subscriptionRef.equalTo(userEmail).addListenerForSingleValueEvent(
    ...
    usersubscriptionRef.addListenerForSingleValueEvent(
        object : ValueEventListener {
            override fun onCancelled(error: DatabaseError) {}
            override fun onDataChange(snapshot: DataSnapshot) {
                snapshot.children.forEach {
...
fun writeImageRef(app: WitParkingApp, imageRef: String) {
    val userId = app.currentUser.uid
...
fun validatePhoto(app: WitParkingApp, activity: Activity) {
...
fun checkExistingPhoto(app: WitParkingApp, activity: Activity) {
