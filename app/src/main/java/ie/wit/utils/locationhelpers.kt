...
fun setCurrentLocation(app: WitParkingApp) {
...
fun trackLocation(app: WitParkingApp) {
    ...
    app.locationClient.requestLocationUpdates(
        createDefaultLocationRequest(),
        locationCallback, null
    )
}

fun setMapMarker(app: WitParkingApp) {
...
fun getAllSubscriptions(app: WitParkingApp) {
    val subscriptionsList = ArrayList<SubscriptionModel>()

    app.database.child("user-subscriptions").child(app.currentUser.uid)
        .addValueEventListener(object : ValueEventListener {
            ...
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                children.forEach {
                    val donation = it.getValue<SubscriptionModel>(SubscriptionModel::class.java)
                    subscriptionsList.add(donation!!)
                }
                addMapMarkers(subscriptionsList, app.mMap)
            }
        })

}

fun getFavouriteSubscriptions(app: WitParkingApp) {
    val subscriptionsList = ArrayList<SubscriptionModel>()
...
fun addMapMarkers(dl: ArrayList<SubscriptionModel>, map: GoogleMap) {
    dl.forEach {
        map.addMarker(
